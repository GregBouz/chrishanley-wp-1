...
<?php 
if( isset( $_GET['propertyid'] ) && isset( $_GET['agentid'] ) ) {
...
if(isset($_POST['submitted'])) {
	...
	if(trim($_POST['checking']) !== '') {
		...
		//Check to make sure that the property name field is not empty
		if ($agent_contact) {
			if (isset($_POST['propertyName'])) {
				if(trim($_POST['propertyName']) === '') {
					$propertyError =  __('You forgot to enter the property name.', 'woothemes'); 
					$hasError = true;
				} else {
					$property_name = trim($_POST['propertyName']);
					$agentid = (int)$_POST['agentid'];
					$agent_contact = true;
					if ($agentid > 0) {
						$author_email = esc_attr( get_the_author_meta( 'user_email', $agentid ) );
					} else {
		...
		if(!isset($hasError)) {
			...
			if ($agent_contact) {
				...
				$subject = __('Property Enquiry from ', 'woothemes').$name;
			} else {
				$emailTo = esc_attr( get_option('woo_contactform_email') );
                ...
                <?php while (have_posts()) : the_post(); ?>
                    ...
                    <form action="<?php the_permalink(); ?>" id="contactForm" method="post">
                        ...
                        <ol class="forms">
                            ...
                            <li><label for="propertyName"><?php _e('Property', 'woothemes'); ?></label>
                                <input type="text" name="propertyName" id="propertyName" value="<?php if(isset($_POST['propertyName'])) { echo $_POST['propertyName']; } else { echo $property_name; }?>" class="txt<?php if ($agent_contact) { ?> requiredField<?php } ?>" />
                                <?php if($propertyError != '') { ?>
                                    <span class="error"><?php echo $propertyError;?></span> 
                                <?php } ?>
